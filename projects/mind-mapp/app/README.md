...
- Search (Cmd/Ctrl+K) with focus+center on selection, ID + path matching, ranked results, path/context metadata, highlighted matches, quoted/negated query terms, Tab cycle + Page/Home-End navigation hints, accurate match counts, and cached path matching
...
- Mini-map navigator (Shift+M toggle; click node to focus + center, click background to recenter, drag viewport box to pan, Arrow/Shift+Arrow pan when mini-map is focused) with live viewport indicator
- Sample map loader
