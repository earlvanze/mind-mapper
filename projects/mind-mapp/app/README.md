...
- Search (Cmd/Ctrl+K toggle + toolbar Search On/Off toggle button with dialog aria-controls/expanded/haspopup wiring + aria-keyshortcuts metadata) with focus+center on selection, ID + path matching, ranked results, path/context metadata, highlighted matches, quoted/negated query terms (including spaced negation before quoted phrases, e.g. - "alpha review", and unicode dash negation markers −/–/—; contradictory include/exclude terms short-circuit to no results), whitespace/diacritic/punctuation/camelCase-normalized phrase matching, Tab cycle + Page/Home-End navigation hints, Shift/Cmd/Ctrl/Alt+Enter/click jump-without-close, Esc clear-then-close + Cmd/Ctrl+Shift+K clear + Cmd/Ctrl+F focus + Cmd/Ctrl+A select-input behavior, accurate match counts, shared default search result cap + cached search index/path matching for repeated queries with one-pass bucketed rank assembly (0-4) + direct Node output to avoid full-match sorting and extra rank-object allocations + deterministic same-rank text/id ordering + single-pass positive/negative token partitioning + loop-based pre-tokenized token normalization/filtering, shared tokenization between search ranking + highlight rendering + cached query-token parsing for repeated identical input + fast-path reuse for already-normalized token arrays + repeated positive/negative term dedupe in rank checks + cached term-partition reuse for repeated token arrays + include-only/exclude-only skip-path branching + pre-sized bucket flattening + direct searchNodes no-wrapper path when total is unused + searchNodesWithTotal zero-limit no-slice path, deferred query evaluation with "updating…" status for smoother large-map typing, shared jump/navigation guards (Enter/click + Tab/Page/Home/End + hover selection) while deferred results are still catching up, with centralized key→selection mapping helper + navigation keys intercepted as no-ops to prevent focus escape/scroll drift, pending-result rows marked aria-disabled with disabled visual state + hover selection disabled + shared updating tooltip (rows + listbox), search empty-state now distinguishes pending ("Searching nodes…") vs settled no-results copy and stays hidden for blank/whitespace-only or tokenless (e.g., punctuation-only) input, shared search summary formatter, WeakMap-cached focus-path resolver for result metadata labels, improved listbox accessibility semantics (summary + hint are wired via aria-describedby; listbox exposes aria-busy + aria-disabled while deferred results are catching up, and combobox active-descendant is paused while pending), and an explicit Close button in the dialog header (with close-key aria-keyshortcuts metadata); dialog container and search input both expose in-dialog keyboard map metadata via aria-keyshortcuts and reference summary/hint via aria-describedby, with registry-alignment regression checks on shared keymap constants
...
- Help dialog (? / Cmd/Ctrl+/) toggle with toolbar Help On/Off state, compact Focus Navigation & History section, and live shortcut filter (punctuation-agnostic + symbol/alias term matching with repeated-term dedupe, including queries like "forward slash" or "question mark"; Esc clear-then-close + Cmd/Ctrl+Shift+K clear + Cmd/Ctrl+F focus + Cmd/Ctrl+A select-input; Cmd/Ctrl+/ works even when typing in dialog inputs; filter summary + hint are wired via aria-describedby) plus an explicit Close button in the dialog header (with close-key aria-keyshortcuts metadata); toolbar Help toggle also advertises dialog popup semantics via aria-haspopup and both dialog container + filter input expose in-dialog keyboard map metadata via aria-keyshortcuts + summary/hint aria-describedby references; Help filtering uses deferred query evaluation with shared summary formatting, cached normalized shortcut haystacks + blank-input short-circuit + 1-term/2-term fast-path matching, aria-busy/aria-disabled pending semantics, and shared pending/no-match empty-state helper copy
...
- Toolbar controls expose shortcut metadata (`aria-keyshortcuts`) across navigation/history/search/help/export/copy/reset actions plus advanced selection/layout actions for assistive-tech discoverability
