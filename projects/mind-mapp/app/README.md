...
- Help dialog (? / Cmd/Ctrl+/) toggle with toolbar Help On/Off state, compact Focus Navigation & History section, and live shortcut filter (punctuation-agnostic + symbol/alias term matching, including queries like "forward slash" or "question mark"; Esc clear-then-close + Cmd/Ctrl+Shift+K clear + Cmd/Ctrl+F focus + Cmd/Ctrl+A select-input; Cmd/Ctrl+/ works even when typing in dialog inputs; filter summary + hint are wired via aria-describedby) plus an explicit Close button in the dialog header (with close-key aria-keyshortcuts metadata); toolbar Help toggle also advertises dialog popup semantics via aria-haspopup and both dialog container + filter input expose in-dialog keyboard map metadata via aria-keyshortcuts + summary/hint aria-describedby references
