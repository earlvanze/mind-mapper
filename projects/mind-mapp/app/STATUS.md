...
- Search + keyboard navigation (Cmd/Ctrl+K toggle + toolbar Search On/Off button with aria-expanded/aria-controls/aria-haspopup + aria-keyshortcuts, centers selected result, supports ID/path multi-term matching + path metadata + normalized term highlighting + quoted/negated terms + whitespace/diacritic/punctuation/camelCase-normalized phrase matching + Tab cycle + PageUp/PageDown + Home/End jumps + Shift/Cmd/Ctrl/Alt+Enter/click jump-without-close + Esc clear-then-close + Cmd/Ctrl+Shift+K clear + Cmd/Ctrl+F focus + Cmd/Ctrl+A select-input + capped-result match counts, parser unified, cached search index/path ranking for repeated queries, shared tokenization between ranking + highlight, normalized non-finite/negative caps for robust result limiting, deferred query evaluation with inline "updating…" status, Enter/click jumps gated until deferred results settle, pending-result rows marked aria-disabled with disabled visual state + hover selection gated + updating tooltip, shared search summary formatter, cached focus-path resolver for result metadata, listbox/active-descendant + posinset/setsize + aria-busy/aria-disabled a11y semantics, summary/hint aria-describedby wiring on input/listbox/dialog, explicit header close button with close-key aria-keyshortcuts, dialog+input aria-keyshortcuts map sourced from shared dialogKeyshortcuts constants with registry-alignment regression checks; shared normalization helper for match/highlight parity)
