...
test('imports a Trello board JSON export as a new mind-map page', async ({ page }) => {
  ...
  expect(todo.x + todo.width / 2).toBeCloseTo(760, 4)
  expect(todo.y + todo.height / 2).toBeCloseTo(130, 4)
  expect(doing.x + doing.width / 2).toBeCloseTo(760, 4)
  expect(doing.y + doing.height / 2).toBeCloseTo(990, 4)
