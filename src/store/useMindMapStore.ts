...
type MindMapState = {
  ...
  reconnectEdge: (fromId: string, oldToId: string, newToId: string) => void;
...
export const useMindMapStore = create<MindMapState>((set, get) => ({
  ...
  setText: (id, text) =>
    set(state => {
      ...
      // Parse HTML for bold/italic tags and sync style booleans
      const hasBold = /<b[^>]*>/i.test(text) || /<strong[^>]*>/i.test(text);
      const hasItalic = /<i[^>]*>/i.test(text) || /<em[^>]*>/i.test(text);

      // Only update style booleans if they differ from current
      const styleUpdate: Partial<NodeStyle> = {};
      if (current.style?.bold !== hasBold) styleUpdate.bold = hasBold;
      if (current.style?.italic !== hasItalic) styleUpdate.italic = hasItalic;

      const needsStyleUpdate = Object.keys(styleUpdate).length > 0;
      const newStyle = needsStyleUpdate
        ? (current.style ? { ...current.style, ...styleUpdate } : styleUpdate)
        : current.style;
  ...
  startConnection: (fromId) => set({ connectMode: true, pendingConnection: { fromId, toId: null } }),
  cancelConnection: () => set({ connectMode: false, pendingConnection: null }),
  completeConnection: (toId) => set(state => {
    if (!state.pendingConnection) return {};
    const fromId = state.pendingConnection.fromId;
    if (toId === fromId || state.nodes[toId]?.parentId === fromId) {
      // Prevent self-loops and direct parent-child duplicates
      return { connectMode: false, pendingConnection: null };
    }
    // Check if edge already exists
    const existingNode = state.nodes[fromId];
    if (existingNode?.children.includes(toId)) {
      return { connectMode: false, pendingConnection: null };
    }
    const updated: Record<string, Node> = {};
    // Add to parent's children
    const parentNode = state.nodes[fromId];
    updated[fromId] = { ...parentNode, children: [...parentNode.children, toId] };
    // Update child's parentId
    const childNode = state.nodes[toId];
    // Remove from old parent if exists
    if (childNode.parentId && state.nodes[childNode.parentId]) {
      const oldParent = state.nodes[childNode.parentId];
      updated[childNode.parentId] = { ...oldParent, children: oldParent.children.filter(c => c !== toId) };
    }
    updated[toId] = { ...childNode, parentId: fromId };
    return { ...withHistory(state), nodes: updated, connectMode: false, pendingConnection: null };
  }),
  ...
  reconnectEdge: (fromId, oldToId, newToId) =>
    set(state => {
      if (fromId === newToId) return {}; // Can't connect to self
      if (oldToId === newToId) return {}; // No change
      const updated = { ...state.nodes };
      const from = updated[fromId];
      const child = updated[oldToId];
      ...
      // Add to new parent
      updated[fromId] = { ...updated[fromId], children: [...updated[fromId].children, newToId] };
      // Update child's parent
      updated[oldToId] = { ...child, parentId: fromId };
      return { ...withHistory(state), nodes: updated, selectedEdgeId: undefined };
    }),
  selectAll: () =>
    set(state => {
  ...
  setNodeStyle: (id, style) =>
    set(state => {
      ...
      // If style is undefined, remove styling
      if (style === undefined) {
        const { style: _, ...rest } = node;
        return {
          ...withHistory(state),
          nodes: { ...state.nodes, [id]: rest as Node },
        };
      }
      
      // Merge with existing style
      const newStyle = node.style ? { ...node.style, ...style } : style;
      
      return {
        ...withHistory(state),
        nodes: { ...state.nodes, [id]: { ...node, style: newStyle } },
      };
    }),
  ...
  addTagToSelected: (tag) =>
    ...
    setNodeComment: (id, comment) => {
      set((state) => {
        ...
        if (!node) return {};
        const newNodes = { ...state.nodes };
        newNodes[id] = { ...newNodes[id], comment };
